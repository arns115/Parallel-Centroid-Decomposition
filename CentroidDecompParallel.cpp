...
int getSize(int curNode, int parent = -1) {
	...
	sz[curNode] = 1;
  // If adjacency list of a specific vertex is big enough go through it using many threads
  if(adj[curNode].size()>10){
  ...
  // If adjacency list of a specific vertex isn't big enough go thorough it using a single thread
  else{
      for (int x : adj[curNode]) {
...
int main(){
  ...
  if(test){
    ...
    fclose(stdin);
    freopen("dataParallel.txt", "w", stdout);
    ...
    freopen("ParallelTest.txt", "w", stdout);
    for (int i=1;i<=T;i++){
      cout << "VERTEX " << i << ":" << endl;
      for (int x:ancestors[i]){
        cout << x << " ";
